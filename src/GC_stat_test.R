...
##test normality of GC
shapiro <- shapiro.test(gc_depth_dna$GC)
...
summary <- group_by(gc_depth_dna, plot_label) %>%
  summarise(
    ...
    IQR = IQR(GC, na.rm = TRUE)
  )
summary_chars <- capture.output(print(summary))
writeLines(summary_chars, con=file(snakemake@output[["summary_stats"]]))

##does GC content between any groups differ significantly
kruskal_res <- kruskal.test(GC ~ plot_label, data = gc_depth_dna)
...
pair_wilcox_res <- pairwise.wilcox.test(gc_depth_dna$GC, gc_depth_dna$plot_label,
                     p.adjust.method = "BH")
