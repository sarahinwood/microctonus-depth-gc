...
shapiro <- shapiro.test(gc_depth$GC)
...
kruskal_res <- kruskal.test(GC ~ plot_label, data = gc_depth)
...
pair_wilcox_res <- pairwise.wilcox.test(gc_depth$GC, gc_depth$plot_label,
                     p.adjust.method = "BH")
